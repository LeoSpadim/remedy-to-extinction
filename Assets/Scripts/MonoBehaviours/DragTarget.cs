...
public class DragTarget : MonoBehaviour
{
	...
	void Update ()
	{
		...
		//if(DialogueUI.Singleton.isShowingDialogue) {return;}

		if (Input.GetMouseButtonDown(0) && !DragUI.isDragging && !PauseMenu.PAUSED)
		{
			// Fetch the first collider.
			// NOTE: We could do this for multiple colliders.
			collider = Physics2D.OverlapPoint (worldPos, m_DragLayers);
			if (!collider)
            {
                return;
            }
		...
		if (m_TargetJoint)
		{
			if(DragUI.isDragging/* || DialogueUI.Singleton.isShowingDialogue*/)
				return;
